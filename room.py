...
class Roombooking:
    def __init__(self,root):
       ...
       lbl_menu= Label(self.root,text="ROOM BOOKING DETAILS",font=("times new roman",18,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE)
       lbl_menu.place(x=0,y=0,width=1295,height=50)
       ...
       #Fetch data button
       btnFetchData=Button(labelframeleft,command=self.Fetch_contact,text="FETCH",font=("arial",9,"bold"),bg="black",fg="gold",width=8)
      ...
      # conn=mysql.connector.connect(host="localhost",user="root",password="Priya@123",database="management")
       ...
       #sub total
       lblMeal= Label(labelframeleft,text="Sub Total",font=("arial",12,"bold"),padx=2,pady=6)
       lblMeal.grid(row=8,column=0,sticky=W)
       txtMeal=ttk.Entry(labelframeleft,textvariable=self.var_actualtotal,font=("arial",13,"bold"),width=29)
       ...
       img3 =Image.open(r"C:\photos\hottal.jpg")
       ...
       self.room_table.column("contact",width=90)
       self.room_table.column("checkinDate",width=90)
       self.room_table.column("checkOutdate",width=90)
       self.room_table.column("roomtype",width=90)
       self.room_table.column("roomavailable",width=90)
       self.room_table.column("meal",width=90)
       self.room_table.column("noOfdays",width=90)
      ...
      # add data 
    def add_data(self):
        if self.var_contact.set()=="" or self.var_checkin.set()=="":
            messagebox.showerror("Error","All fields are required",parent=self.root)
        else:
            try:
               conn=mysql.connector.connect(host="localhost",user="root",password="Priya@123",database="management")
               my_cursor=conn.cursor()
               my_cursor.execute("insert into new_table values(%s,%s,%s,%s,%s,%s,%s)",(
   ...
   # ========= fetch data =============================
    ...
    def update(self):
        if self.var_contact.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",user="root",password="Priya@123",database="management")
            my_cursor=conn.cursor()
            my_cursor.execute("update new_table set check_in=%s, check_out=%s,roomtype=%s,roomavailable=%s,meal=%s,noOfdays=%s where Contact=%s",(
    ...
    def mdelete(self):
         ...
         if mdelete>0:
            conn=mysql.connector.connect(host="localhost",user="root",password="Priya@123",database="management")
            my_cursor=conn.cursor()
            query="delete from new_table where Contact=%s"
    ...
    def Fetch_contact(self):
      if self.var_contact.set()=="":
         messagebox.showerror("Error","Please Enter Contact Numbar",parent=self.root)
      else:
         conn=mysql.connector.connect(host="localhost",user="root",password="Priya@123",database="management")
         my_cursor=conn.cursor()
         query=("select name from customer where mobile=%s")
         value=(self.var_contact.set(),)
         my_cursor.execute(query,value)
         ...
         if row==None:
            ...
            showDataframe.place(x=450,y=55,weight=300,height=180)
    ...
    def search(self):
        ...
        my_cursor.execute("select* from new_table where"+str(self.search_var.get())+ "LIKE'%"+str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len(rows)!=0:
            self.room_table.delete(*self.room_table.get_children())
            for i in rows:
